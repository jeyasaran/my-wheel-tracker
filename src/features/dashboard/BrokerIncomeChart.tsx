...
export default function BrokerIncomeChart() {
    const { trades, stockPositions, brokers } = useTradeStore();
    ...
    const chartData = useMemo(() => {
        ...
        filtered.forEach(item => {
            ...
            const brokerId = (item as any).brokerId || '__unassigned__';
            const brokerName = brokers.find(b => b.id === brokerId)?.name || 'Unassigned';

            const entry = groupedData.get(key)!;
            if (!entry[brokerName]) entry[brokerName] = 0;

            entry[brokerName] += pnl;
            entry.total += pnl;
        });
        ...
        return result;
    }, [trades, stockPositions, period, viewType, timeRange, brokers]);

    const activeBrokers = useMemo(() => {
        const brokerSet = new Set<string>();
        chartData.forEach(entry => {
            Object.keys(entry).forEach(key => {
                if (key !== 'date' && key !== 'dateObj' && key !== 'total' && key !== 'cumulative') {
                    brokerSet.add(key);
                }
            });
        });
        ...
        <Card className="w-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div className="space-y-1">
                    <CardTitle className="text-base font-normal text-muted-foreground">Income by Broker</CardTitle>
                    <div className={cn(
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        {viewType === 'Bar' ? (
                            <BarChart data={chartData}>
                                ...
                                />
                                <Legend />
                                {activeBrokers.map((brokerName, index) => {
                                    const isTopBar = index === activeBrokers.length - 1;
                                    return (
                                        <Bar
                                            key={brokerName}
                                            dataKey={brokerName}
                            ...
                            <AreaChart data={chartData}>
                                <defs>
                                    <linearGradient id="colorCumulativeBroker" x1="0" y1="0" x2="0" y2="1">
                                ...
                                <Area
                                    ...
                                    strokeWidth={2}
                                    fillOpacity={1}
                                    fill="url(#colorCumulativeBroker)"
                                    name="Cumulative P&L (All Brokers)"
                                />
                            </AreaChart>
                        )}
                    </ResponsiveContainer>
